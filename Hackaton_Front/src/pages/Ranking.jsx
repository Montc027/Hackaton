    ...
    const { rankingSaturados, rankingTranquilos } = useMemo(() => {
        ...
        counts[barrio] = (counts[barrio] || 0) + 1;
        });

        const arr = Object.entries(counts).map(([barrio, peso]) => ({ barrio, peso }));
    ...
    const renderItem = (r, tipo) => {
        const isTop = r.posicion === 1;
        const bgClass = tipo === "tranquilo" ? "bg-green-300" : "bg-red-300";
        const img = tipo === "tranquilo" ? "/barcelona_ciudad.jpg" : "/bcn_saturados.jpg";
        ...
        <motion.div
            key={r.barrio}
            className={`flex justify-between items-center p-3 rounded-lg shadow-md ${bgClass}`}
        ...
        >
            ...
            <span className="font-mono underline decoration-dotted cursor-help" title={descripcionScore}>
            {r.peso}
            </span>
        </motion.div>
        );
    ...
    return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
            <h3 className="text-xl font-bold mb-2 text-center text-green-600">Top 5 Barrios más tranquilos</h3>
            <motion.div className="flex flex-col gap-3">
                {rankingTranquilos.map((r) => renderItem(r, "tranquilo"))}
            </motion.div>
            </div>

            <div>
            <h3 className="text-xl font-bold mb-2 text-center text-red-600">Top 5 Barrios más saturados</h3>
            <motion.div className="flex flex-col gap-3">
                {rankingSaturados.map((r) => renderItem(r, "saturado"))}
            </motion.div>
            </div>
        </div>
        </div>
